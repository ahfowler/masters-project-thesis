...
window.onload = function() {
    ...
    function drawFrame() {
        ...
        context.fillText("Make a fist to move the arrow. Release into an open hand to shoot.", 13, 57);
        
        // // Display fps
        // context.fillStyle = "#ffffff";
        // context.font = "12px Verdana";
        // context.fillText("Fps: " + fps, 13, 87);
    }
    ...
    // On mouse movement
    onMouseMove = function onMouseMove(cursorPosition) {
        // Get the mouse position
        var pos = getMousePos(cursorPosition);
    ...
    onMouseDown = function onMouseDown(cursorPosition) {
        // Get the mouse position
        var pos = getMousePos(cursorPosition);
    ...
    // Get the mouse position
    function getMousePos(cursorPosition) {
        var rect = canvas.getBoundingClientRect();

        return {
            x: Math.round((cursorPosition.x - rect.left)/(rect.right - rect.left)*canvas.width),
            y: Math.round((cursorPosition.y - rect.top)/(rect.bottom - rect.top)*canvas.height)
        };
    }
};